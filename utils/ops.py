    ...
    while total_pix < n_drop_pix:
        ...
        if x_rep is None:
            x_uninitialized = torch.empty(
                (c, rnd_h - rnd_r, rnd_w - rnd_c), dtype=x.dtype, device=args.device
            ).normal_()
            if torch.max(x_uninitialized) - torch.min(x_uninitialized) != 0:
...
def rot_rand(args, x_s):
    ...
    x_rot = torch.zeros(img_n).long().to(device=args.device)
